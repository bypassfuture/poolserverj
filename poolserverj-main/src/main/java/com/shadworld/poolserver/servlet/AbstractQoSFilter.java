 ...
 * "maxRequests" init parameter (default 10). If more requests are received,
 * they are suspended and placed on priority queues. Priorities are determined
 * by the {@link #getPriority(ServletRequest)} method and are a value between 0
 * and the value given by the "maxPriority" init parameter (default 10), with
 * higher values having higher priority. </p>
 * <p>
 * This filter is ideal to prevent wasting threads waiting for slow/limited
 * resources such as a JDBC connection pool. It avoids the situation where all
 * of a containers handler pool may be consumed blocking on such a slow resource.
 * By limiting the number of active threads, a smaller handler pool may be used
 * as the threads are not wasted waiting. Thus more memory may be available for
 * use by the active threads.
 * </p>
 * <p>
 * Furthermore, this filter uses a priority when resuming waiting requests. So
 * that if a container is under load, and there are many requests waiting for
 * resources, the {@link #getPriority(ServletRequest)} method is used, so that
 * more important requests are serviced first. For example, this filter could be
 * deployed with a maxRequest limit slightly smaller than the containers handler
 * pool and a high priority allocated to admin users. Thus regardless of load,
 * admin users would always be able to access the web application.
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * If the "managedAttr" init parameter is set to true, then this servlet is set
 * as a {@link ServletContext} attribute with the filter name as the attribute
 * name. This allows context external mechanism (eg JMX via
 * {@link ContextHandler#MANAGED_ATTRIBUTES}) to manage the configuration of the
 * filter.
 * </p>
 * 
 * 
 */
public class AbstractQoSFilter implements Filter {
	 ...
	 * </ul>
	 * This method may be specialised to provide application specific
	 * priorities.
	 * 
	 * @param request
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get the (short) amount of time (in milliseconds) that the filter would
	 * wait for the semaphore to become available before suspending a request.
	 * 
	...
	/* ------------------------------------------------------------ */
	/**
	 * Set the (short) amount of time (in milliseconds) that the filter would
	 * wait for the semaphore to become available before suspending a request.
	 * 
	 * @param value
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get the amount of time (in milliseconds) that the filter would suspend a
	 * request for while waiting for the semaphore to become available.
	 * 
	...
	/* ------------------------------------------------------------ */
	/**
	 * Set the amount of time (in milliseconds) that the filter would suspend a
	 * request for while waiting for the semaphore to become available.
	 * 
	 * @param value
	...
	/* ------------------------------------------------------------ */
	/**
	 * Get the maximum number of requests allowed to be processed at the same
	 * time.
	 * 
	...
	/* ------------------------------------------------------------ */
	/**
	 * Set the maximum number of requests allowed to be processed at the same
	 * time.
	 * 
	 * @param value
